...
const INSERT_SQL = `INSERT INTO applications (
  ...
  partner_key, extra_json, status, note
) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'new',?)`;
 ...
 */
export async function recordApplication(db: D1Database | undefined, input: RecordApplicationInput): Promise<boolean> {
  if (!db) return false;
  try {
    ...
    ? db
        .prepare(
          `SELECT id, created_at, kind, status, slug, public_url, store_name, business_type, catchcopy,
                  ...
                  has_photo, owner_email, partner_key, note
...
const CSV_HEADERS: Array<{ key: keyof ApplicationRow; label: string }> = [
  ...
  { key: "slug", label: "スラッグ" },
  { key: "partner_key", label: "紹介パートナー鍵" },
  { key: "note", label: "メモ" },
];
